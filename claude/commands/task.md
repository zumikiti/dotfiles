...
2. **タスク追加**（キーワード: 追加、add）
   ...
   - パラメータ: title, status?（デフォルト: "planned", icebox/planned/in-progress/on-hold から選択）, start?, due?, review?, body?（自由形式のメモ：作業ログ・進捗メモ・設計メモ・参考URLなど）, externalLinks?（日報への参照リンク `[{title, url}]` 形式。日報の該当日にジャンプする用途で使用）
...
5. **タスク更新**（キーワード: 更新、update、変更）
   ...
   - パラメータ: title, newTitle?, status?（icebox/planned/in-progress/on-hold/done/cancelled）, start?, due?, review?, body?, addLink?（日報リンクURL）, linkText?（日報リンクのテキスト）
   - タイトルが曖昧な場合は `mcp__dailymemo__list_tasks` で一覧を表示し、ユーザーに対象を選択してもらう
