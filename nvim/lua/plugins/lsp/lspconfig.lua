return {
  ...
  config = function()
    ...
    -- mason-lspconfig v2 では `handlers` API が廃止され、
    -- インストール済みサーバーは automatic_enable(vim.lsp.enable)で有効化される。
    -- サーバー設定は Neovim 0.11+ の `vim.lsp.config` で行う。
    mason_lspconfig.setup({})

    -- 全サーバー共通の既定設定
    vim.lsp.config("*", {
      capabilities = capabilities,
      -- インクリメンタル同期のバッファ追跡がズレると sync.lua:195 で
      -- assert が落ちるバグ(Neovim 0.12.x)を回避するため、フル同期を強制する。
      flags = {
        allow_incremental_sync = false,
      },
    })

    -- emmet はデフォルトの filetype を上書き
    vim.lsp.config("emmet_ls", {
      filetypes = { "html", "typescriptreact", "javascriptreact", "css", "sass", "scss", "less", "svelte" },
    })

    -- lua_ls は `vim` グローバルを認識させる
